...
/**
 * Tests of the templates and mappers
 */
@SpringBootTest
...
public class RepositoryTests {
	...
	public void testRedisHash() {
		...
		byte[] bytes = connection.hashCommands().hGet(key.getBytes(), scoreField.getBytes());
		try {
		...
		connection.hashCommands().hIncrBy(key.getBytes(), scoreField.getBytes(), 199);
		connection.hashCommands().hIncrBy(key.getBytes(), scoreField.getBytes(), -29);

		bytes = connection.hashCommands().hGet(key.getBytes(), scoreField.getBytes());
		System.out.println("native connection get = " + bytes);
